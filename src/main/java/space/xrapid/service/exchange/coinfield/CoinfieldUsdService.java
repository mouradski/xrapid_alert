...
import space.xrapid.domain.exchange.coinfield.Trades;
...
public class CoinfieldUsdService implements TradeService {
    ...
    private List<Trade> getTrades(OffsetDateTime begin, ResponseEntity<Trades> response) {
        return response.getBody().getTrades().stream()
                .sorted(Comparator.comparing(space.xrapid.domain.exchange.coinfield.Trade::getTimestamp))
                .map(this::mapTrade)
                .filter(filterTradePerDate(begin))
                .collect(Collectors.toList());
    }

    private Trade mapTrade(space.xrapid.domain.exchange.coinfield.Trade trade) {
