...
public class MercadoBitcoinService implements TradeService {
    ...
    public List<Trade> fetchTrades(OffsetDateTime begin) {
        ...
        ResponseEntity<space.xrapid.domain.exchange.mercadobitcoin.Trade[]> response = restTemplate
                .exchange(apiUrl.replace("{FROM}", begin.toEpochSecond() + "")
                                .replace("{TO}", OffsetDateTime.now(ZoneOffset.UTC).toEpochSecond() + ""),
                        HttpMethod.GET, entity, space.xrapid.domain.exchange.mercadobitcoin.Trade[].class);
    ...
    private Trade mapTrade(space.xrapid.domain.exchange.mercadobitcoin.Trade trade) {
