...
import space.xrapid.domain.xumm.Amount;
import space.xrapid.domain.xumm.CustomMeta;
import space.xrapid.domain.xumm.Payload;
import space.xrapid.domain.xumm.PaymentRequestInformation;
import space.xrapid.domain.xumm.Response;
import space.xrapid.domain.xumm.XummPayment;
import space.xrapid.domain.xumm.XummPaymentStatus;
...
public class XummService {
  ...
  public PaymentRequestInformation requestPayment(double amount, String currency, Long days,
      String key) {
    ...
    String instruction = "Your key will be available after payment confirmation. Your key will expire : _EXPIRE_."
        .replace("_EXPIRE_", expiration.toString());
    ...
    ResponseEntity<Response> response = restTemplate.exchange(apiBase,
        HttpMethod.POST, new HttpEntity(buildPayment(finalAmount, currency, instruction), headers),
        Response.class);
    ...
    return PaymentRequestInformation.builder().paymentId(id)
        .qrCodeUrl(response.getBody().getRefs().getQrPng()).build();
  }
  ...
  public void updatePaymentStatus(WebHook webHook) {
    ...
    if (webHook.getPayloadResponse().getSigned() == null || !webHook.getPayloadResponse()
        .getSigned()) {
      status.put(id, "REJECTED");
    } else if (webHook.getPayloadResponse().getSigned() != null && webHook.getPayloadResponse()
        .getSigned()) {
      HttpHeaders headers = getHeaders();
      ...
      if (response.getBody().getResponse().getAdditionalProperties()
          .containsKey("dispatched_result")
          && response.getBody().getResponse().getAdditionalProperties().get("dispatched_result")
          .toString().startsWith("tec")) {
        status.put(id, "REJECTED");
      } else {
        if (keys.get(id) != null) {
