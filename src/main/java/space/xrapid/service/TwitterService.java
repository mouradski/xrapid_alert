  ...
  public void dailySummary(GlobalStats globalStats) {
    ...
    lastDayStats.entrySet().stream()
      ...
      sb.append("From ").append(volumePerCorridor.getKey().split("-")[0])
          ...
          .append(" :  ").append(
          NumberFormat.getCurrencyInstance(Locale.US).format(volumePerCorridor.getValue())
              .replaceAll("\\.[0-9]{2}", ""))
          .append("\n");
    });

    sb.append("\n").append("ODL daily volume :  ")
        .append(NumberFormat.getCurrencyInstance(Locale.US)
            .format(lastDayVolume).replaceAll("\\.[0-9]{2}", ""))
        .append("\n\n");

    sb.append("Last Daily ATH :  ").append(
        NumberFormat.getCurrencyInstance(Locale.US).format(globalStats.getDailyAth())
            .replaceAll("\\.[0-9]{2}", ""));
