...
public abstract class XrapidCorridors {
  ...
  public XrapidCorridors(ExchangeToExchangePaymentService exchangeToExchangePaymentService,
      TradesFoundCacheService tradesFoundCacheService,
      XrapidInboundAddressService xrapidInboundAddressService,
      SimpMessageSendingOperations messagingTemplate, List<Exchange> exchangesToExclude,
      Set<String> usedTradeIds, String proxyUrl) {
    ...
    allExchangeAddresses = Arrays.stream(Exchange.values()).map(Exchange::getAddresses)
  ...
  protected void persistPayment(ExchangeToExchangePayment exchangeToFiatPayment) {
    try {
      ...
      if (exchangeToFiatPayment.getFiatToXrpTrades() != null && !exchangeToFiatPayment
          .getFiatToXrpTrades().isEmpty()) {
        exchangeToFiatPayment.setSourceFiat(
            exchangeToFiatPayment.getFiatToXrpTrades().get(0).getExchange().getLocalFiat());
      ...
      if (exchangeToFiatPayment.getXrpToFiatTrades() != null && !exchangeToFiatPayment
          .getXrpToFiatTrades().isEmpty()) {
        exchangeToFiatPayment.setDestinationFiat(
            exchangeToFiatPayment.getXrpToFiatTrades().get(0).getExchange().getLocalFiat());
      } else {
        exchangeToFiatPayment
            .setDestinationFiat(exchangeToFiatPayment.getDestination().getLocalFiat());
      }

      if (exchangeToFiatPayment.getDestinationFiat() != null &&
          exchangeToFiatPayment.getDestinationFiat()
              .equals(exchangeToFiatPayment.getSourceFiat())) {
        return;
      }
      ...
      if (exchangeToExchangePaymentService.save(exchangeToFiatPayment)) {
        if (SpottedAt.SOURCE_AND_DESTINATION.equals(exchangeToFiatPayment.getSpottedAt())
            && xrapidInboundAddressService != null) {
          xrapidInboundAddressService.add(exchangeToFiatPayment);
          log.info("{}:{} added as ODL destination candidate.",
              exchangeToFiatPayment.getDestinationAddress(), exchangeToFiatPayment.getTag());
        }
  ...
  private Predicate<Trade> filterFiatToXrpTradePerDate(
      ExchangeToExchangePayment exchangeToExchangePayment) {
    return trade -> {
      long diff = Math.abs(
          ChronoUnit.SECONDS.between(exchangeToExchangePayment.getDateTime(), trade.getDateTime()));
      return exchangeToExchangePayment.getDateTime().isAfter(trade.getDateTime()) && diff < buyDelta
          && diff >= 2;
    };
  }

  protected Predicate<Trade> filterXrpToFiatTradePerDate(
      ExchangeToExchangePayment exchangeToExchangePayment) {
    return trade -> {
      long diff = Math.abs(
          ChronoUnit.SECONDS.between(trade.getDateTime(), exchangeToExchangePayment.getDateTime()));
      return exchangeToExchangePayment.getDateTime().isBefore(trade.getDateTime())
          && diff < buyDelta && diff >= 2;
    };
  }
  ...
  protected boolean xrpToFiatTradesExists(ExchangeToExchangePayment exchangeToExchangePayment) {
    ...
    exchangeToExchangePayment
        .setDestinationCurrencry(exchangeToExchangePayment.getDestinationFiat());
    ...
    Arrays.asList(getAggregatedSellTrades(exchangeToExchangePayment, "sell"),
      ...
      if (!aggregatedTrades.isEmpty() && (exchangeToExchangePayment.getXrpToFiatTrades() == null
          || exchangeToExchangePayment.getXrpToFiatTrades().isEmpty())) {

        List<Trade> closestTrades = tradesFoundCacheService
            .getXrpToFiatTrades(exchangeToExchangePayment.getTransactionHash(),
                aggregatedTrades.get(0).getExchange());

        if (closestTrades == null) {
          closestTrades = TradesCombinaisonsHelper
              .getTrades(aggregatedTrades, exchangeToExchangePayment.getAmount(), "sell");
          if (!closestTrades.isEmpty()) {
            tradesFoundCacheService
                .addXrpToFiatTrades(exchangeToExchangePayment.getTransactionHash(),
        ...
        if (sum > 0) {
          ...
          exchangeToExchangePayment.setXrpToFiatTradeIds(
          ...
          tradesIdAlreadyProcessed
              .addAll(closestTrades.stream().map(Trade::getOrderId).collect(Collectors.toList()));
        }
      }
    });

    if (!exchangeToExchangePayment.isInTradeFound() && SpottedAt.SOURCE_AND_DESTINATION
        .equals(getSpottedAt())) {
  ...
  protected List<Trade> getAggregatedBuyTrades(ExchangeToExchangePayment exchangeToExchangePayment,
      String side) {
